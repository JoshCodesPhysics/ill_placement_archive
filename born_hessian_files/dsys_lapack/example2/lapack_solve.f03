program lapack_solve
      ...
      inquire(file='disp.sols.cart',exist=exist)
      if(exist) then
              open(nout,file='disp.sols.cart',status='old')
      else
              open(nout,file='disp.sols.cart',status='new')
      end if
      if(info==0) then
              !Print the solution
