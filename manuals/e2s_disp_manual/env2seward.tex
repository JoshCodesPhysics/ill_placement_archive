...
\begin{itemize}
	...
	\item Before running the file, make sure the script is in the same directory as the input files described below
	...
	\item Typing `\texttt{python3 env2seward.py}' in the terminal running in the same directory as the python file should execute the script, generating the prompts in the command line. The new file will appear in the same directory as the python script.
\end{itemize} 
...
\end{itemize}
Remember that for the prompt option, the input files must be in the same directory as the env2seward script.
...
Make sure this file is in the same directory as the python script if you are giving the relative path (filename) or if not, quote the full path in the command line when calling it as explained below. This is also the case for the ENV files. Write the full path of these files instead of their filenames in the input file being passed to the command line, if they are not local. 
